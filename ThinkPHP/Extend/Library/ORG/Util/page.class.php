<?php
	class Page {
		...
		private function firstprev(){
			if($this->page > 1) {
				$str="&nbsp;<a href='__ACTION__/page/1'>{$this->config["first"]}</a>";
				$str.="&nbsp;<a href='__ACTION__/page/".($this->page-1)."'>{$this->config["prev"]}</a>&nbsp;";		
				return $str;
			}
		...
		private function pageList(){
			...
			for($i=$inum; $i>=1; $i--){
				...
				if($page>=1)
					$linkPage.="<a href='__ACTION__/page/{$page}'>{$page}</a>&nbsp;";
		...
		private function nextlast(){
			if($this->page != $this->pageNum) {
				$str="&nbsp;<a href='__ACTION__/page/".($this->page+1)."'>{$this->config["next"]}</a>&nbsp;";
				$str.="&nbsp;<a href='__ACTION__/page/".($this->pageNum)."'>{$this->config["last"]}</a>&nbsp;";
				return $str;
			}
